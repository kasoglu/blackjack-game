...
# deal_cards() function that uses the List below to *return* a random card.
def deal_cards():
...
def play_game():
  ...
  while not is_game_over:
    ...
    if user_score == 0 or computer_score == 0 or user_score > 21:
      ...
      # If the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_cards() function to add another card to the user_cards List. If no, then the game has ended.
  ...
  #Once the user is done, it's time to the computer play. The computer should keep drawing cards as long as it has a score less than 17.
...
#After previous game is done restart the game. 
